...
class proces_HDF(object):
    ...
    def copy2servers(self,  afile):
        ...
        servers = [
            r'\\GOA\Shared\AF\Wildfire_Geospatial\Temporary_File_Sharing\NDSI\data',
           r'\\GOA\Shared\AF\Wildfire_Geospatial\RS_Task_Workspaces\NDSI\ARCHIVE',
        ...
        serv_names = ['wildfireSpecialist','bobo_archive','Wildfire', 'FEWS DEV server', 'FEWS UAT server Edmonton', 'FEWS UAT server Calgary',
    ...
    def proc_MOD35_HDF(self, path):
        ...
        self.mkdir(mosaic)
        # MOD35 layers......
        cloud_spi = path_geo + '/cloud_spi'  # folder for Cloud SPI mask
        cloud = path_geo + '/cloud'  # folder for cloud
        qa = path_geo + '/qa'  # folder for cloudQuality_Assurance

        self.mkdir(cloud_spi)
        self.mkdir(cloud)
        self.mkdir(qa)
        ...
        for afile in hdf_List:
            ...
            selected_layers = [ "Cloud_Mask_SPI", "Cloud_Mask", "Quality_Assurance"]

            ##MOD35 layers
            # We will store the data in a dictionary
            # Initialise an empty dictionary
            data = {}
            file_template = 'HDF4_EOS:EOS_SWATH:"%s":mod35:%s'
            #                HDF4_EOS:EOS_SWATH:"MOD35_L2.A2021022.1725.061.2021023010858.hdf":mod35:Scan_Start_Time
        ...
        #         # extract date from the file
        #         # adate = self.getdate_from_hdf_jd(out_84)  # Check this
        #         #get date_from_hdf_jd() has 2 outputs- name and index.  Index indicates part in the file name
        #         #identifying the date of the scene
        #         adate = self.getDate_from_hdfJD(out_84,2)  # Check this
        #
        #         print("adate:", adate)
        #     #
        #     try:
        ...
        #         # copy the same products of 4 tiles into folders for mosaicking
        #
        #         cloud_spi_files= [shutil.copy2(x, os.path.join(cloud_spi, os.path.basename(x))) for x in list_84 if
        #                     'Cloud_Mask_SPI_wgs84' in x]
        #         cloud_files = [shutil.copy2(x, os.path.join(cloud, os.path.basename(x))) for x in list_84 if
        #                     'Cloud_Mask_wgs84' in x]
        #         qa_files = [shutil.copy2(x, os.path.join(qa, os.path.basename(x))) for x in list_84 if
        #                    'Quality_Assurance_wgs84' in x]
        #
        #     except Exception as e:
        #         print("Exception: ", e)
        #         exit(1)
        ...
        #
        # # mosaic all 4 tiles covering AB
        # mos_cloud = os.path.join(mosaic, ymd + "_cloud.tif")
        # mos_cloud_spi = os.path.join(mosaic, ymd + '_cloud_spi.tif')  #
        # mos_qa = os.path.join(mosaic, ymd + "_qa.tif")
        #
        # gdal.BuildVRT(mos_cloud, glob.glob(cloud + "/*.tif"))
        # gdal.BuildVRT(mos_cloud_spi, glob.glob(cloud_spi + "/*.tif"))
    ...
    def proc_MOD11_HDF(self, path):
        ''' processing _land surface tempereature hdf files'''
        os.chdir(path)
        # hdf_folder = path + '/hdf'
        ...
        lst_d = path_geo + '/LST_Day'  # folder for day lst
        angle = path_geo + '/view_anglD'  # folder for sun angle
        lst_n = path_geo + '/LST_Night'  # folder for night lst
        emis_1 = path_geo + '/emis31'  # folder for emis 31
        emis_2 = path_geo + '/emis32'  # folder for emis32
        clear = path_geo + '/clear_cov'
        self.mkdir(lst_d)
        self.mkdir(angle)
        self.mkdir(lst_n)
        self.mkdir(emis_1)
        ...
        for afile in hdf_List:
            ...
            #
            # Let's create a list with the selected layer names
            selected_layers = ["LST_Day_1km","Day_view_angl","LST_Night_1km","Emis_31","Emis_32","Clear_day_cov"]

             ##MOD10A1 layers
            # We will store the data in a dictionary
            # Initialise an empty dictionary
            data = {}
            file_template = 'HDF4_EOS:EOS_GRID:"%s":MODIS_Grid_Daily_1km_LST:%s'
        ...
        #
            try:
                ...
                list_84 = glob.glob(path_geo + '/*tif')
                # copy the same products of 4 tiles into folders for mosaicking
                angle_files = [shutil.copy2(x, os.path.join( angle , os.path.basename(x))) for x in list_84 if
                                 'Day_view_angl_wgs84' in x]

                lstn_files = [shutil.copy2(x, os.path.join( lst_n , os.path.basename(x))) for x in list_84 if
                                 'Night_1km_wgs84' in x]
                lstd_files = [shutil.copy2(x, os.path.join(lst_d, os.path.basename(x))) for x in list_84 if
                          'Day_1km_wgs84' in x]
                emis1_files = [shutil.copy2(x, os.path.join(emis_1, os.path.basename(x))) for x in list_84 if
                          'Emis_31_wgs84' in x]
                emis2_files = [shutil.copy2(x, os.path.join(emis_2, os.path.basename(x))) for x in list_84 if
                           'Emis_32_wgs84' in x]
                clear_files = [shutil.copy2(x, os.path.join(clear, os.path.basename(x))) for x in list_84 if
                           'Clear_day_cov_wgs84' in x]
        ...
        # mosaic all 5 tiles covering AB

        mos_angle = os.path.join(mosaic, ymd + '_angle.tif')  #
        mos_lstn = os.path.join(mosaic, ymd + "_lstn.tif")
        mos_lstd = os.path.join(mosaic, ymd + '_lstd.tif')  #
        mos_emis1 = os.path.join(mosaic, ymd + "_emis1.tif")
        mos_emis2 = os.path.join(mosaic, ymd + '_emis2.tif')  #
        mos_clear = os.path.join(mosaic, ymd + "_clear.tif")
        #
        gdal.BuildVRT(mos_lstd, glob.glob(lst_d + "/*.tif"))
        gdal.BuildVRT(mos_lstn, glob.glob(lst_n + "/*.tif"))
        gdal.BuildVRT(mos_angle, glob.glob(angle +"/*.tif"))
        gdal.BuildVRT(mos_clear, glob.glob(clear +"/*.tif"))
        gdal.BuildVRT(mos_emis1, glob.glob(emis_1 + "/*.tif"))
        gdal.BuildVRT(mos_emis2, glob.glob(emis_2 + "/*.tif"))
    ...
    def proc_MOD10A1F_HDF(self, path):
        ...
        print("ymd:", ymd)
        #date_file = 'C:/_LOCALdata/prj_2021/SNOW_modis/ARCHIVE.dates'
        date_file = r'U:/RS_Task_Workspaces/NDSI/ARCHIVE.dates'
        # ##ISLJUJCI/  UKLJUCI
        print("updating dates file....")
        with open(date_file, 'a') as f:
            # f.write(ymd + '\n')
       ...
       # gdal.BuildVRT(mos_sc, glob.glob(sc + "/*.tif"))
        ...
        # apply colors to a CFG file
        self.aaply_colors(dst)
        #copy colored file into Arhcive
        color_snow = os.path.basename(outclip).replace("Class", "Class_col")
        shutil.copy2(dst, os.path.join(r'U:\RS_Task_Workspaces\NDSI\ARCHIVE', color_snow))
...
#set up an empty directory which will have only one directory and when production is done it will be purged
#then we do not have to code it hard

path = r"U:\RS_Task_Workspaces\NDSI\data"
fold = [x[0] for x in os.walk(path)]
print("New data are in foler: ",fold[1])
# dir4proc = next(os.walk('.'))[1]
# print(dir4proc)
#*************************************
path_MOD10A1F = fold[1]
cfg = proces_HDF()
...
#communitcate to others by sending an email that data have been uploaded to various servers
#NOTE!!! exclude mapling in the subject of the email
os.chdir(r'U:/RS_Task_Workspaces/NDSI/scripts')
com = "python final_goa_1.py"
os.system(com)

#clean up directorey
#We keep only latest folder and it is only one wna we keep 'data' folder empty
